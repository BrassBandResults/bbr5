...
public class ContestResultSql {
    ...
    private static final String PERSON_CONDUCTING_SQL = """
        WITH R AS (
           ...
           FROM contest_result r
           WHERE r.result_position_type <> 'W' AND r.conductor_id = ?1
           UNION
           SELECT r.id, r.contest_event_id, r.band_id
           FROM contest_result r
           WHERE r.result_position_type <> 'W' AND r.conductor_two_id = ?1
           UNION
           SELECT r.id, r.contest_event_id, r.band_id
           FROM contest_result r
           WHERE r.result_position_type <> 'W' AND r.conductor_three_id = ?1
         )
    ...
    private static final String CONTEST_EVENTS_LIST_SQL = """
            ...
            FROM contest_event e
                ...
                LEFT OUTER JOIN person con3 ON con3.id = r.conductor_three_id
            WHERE c.id = ?1
            AND e.date_of_event < GETDATE()
            ORDER BY e.date_of_event DESC""";

    public static List<ContestEventResultSqlDto> selectPastEventListForContest(EntityManager entityManager, Long contestId) {
