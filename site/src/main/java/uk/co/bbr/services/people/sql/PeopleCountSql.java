...
public class PeopleCountSql {
    ...
    private static final String PEOPLE_DONE_NOTHING = """
        ...
        0, 0, 0, 0, 0, 0
        FROM person p
        WHERE NOT EXISTS (SELECT * FROM contest_result r1 WHERE r1.conductor_id = p.id)
        AND NOT EXISTS (SELECT * FROM contest_result r2 WHERE r2.conductor_two_id = p.id)
        AND NOT EXISTS (SELECT * FROM contest_result r3 WHERE r3.conductor_three_id = p.id)
        AND NOT EXISTS (SELECT * FROM contest_event_adjudicator a WHERE a.person_id = p.id)
        AND NOT EXISTS (SELECT * FROM piece p1 WHERE p1.composer_id = p.id)
        AND NOT EXISTS (SELECT * FROM piece p1 WHERE p1.arranger_id = p.id)
        ORDER BY p.surname, p.first_names, p.suffix""";
