...
class ContestListPageContestAliasTests implements LoginMixin {
    ...
    void testFetchContestListWithPrefixWorksSuccessfullyForContestAliases() {
        ...
        assertEquals("Ab Contest 1", pageData.get(2).getName());
        assertEquals("ab-contest-1", pageData.get(2).getSlug());
        assertEquals(0, pageData.get(2).getEventCount());

        assertEquals("Ac Contest 2", pageData.get(3).getName());
        assertEquals("ac-contest-2", pageData.get(3).getSlug());
    ...
    void testFetchContestListLAllWorksSuccessfullyForContestAliases() {
        ...
        assertEquals("Bx Alias 1", pageData.get(5).getName());
        assertEquals("ab-contest-1", pageData.get(5).getSlug());
        assertEquals(0, pageData.get(5).getEventCount());

        assertEquals("Bx Alias 5", pageData.get(6).getName());
        assertEquals("cx-contest-5", pageData.get(6).getSlug());
        assertEquals(0, pageData.get(6).getEventCount());

        assertEquals("Bx Contest 3", pageData.get(7).getName());
        assertEquals("bx-contest-3", pageData.get(7).getSlug());
        assertEquals(0, pageData.get(7).getEventCount());

        assertEquals("Bx Contest 4", pageData.get(8).getName());
        assertEquals("bx-contest-4", pageData.get(8).getSlug());
        assertEquals(0, pageData.get(8).getEventCount());

        assertEquals("Cx Alias 6", pageData.get(9).getName());
        assertEquals("aa-contest-6", pageData.get(9).getSlug());
        assertEquals(0, pageData.get(9).getEventCount());

        assertEquals("Cx Contest 5", pageData.get(10).getName());
        assertEquals("cx-contest-5", pageData.get(10).getSlug());
        assertEquals(0, pageData.get(10).getEventCount());
    }
}
